...
import {
  Box, Typography, Paper, Grid, Chip, Button, TextField, Table,
  TableHead, TableRow, TableCell, TableBody, CircularProgress, Alert,
} from '@mui/material';
...
const availabilityColor = { AVAILABLE: 'success', BOOKED: 'info', EXPIRED: 'default' };
  ...
  return (
    <Box>
      ...
      <Typography variant="h5" fontWeight={700} gutterBottom>
        Truck {truck.truckNo}
        <Chip sx={{ ml: 2 }} label={truck.status} color={statusColor[truck.status] || 'default'} />
      </Typography>
        ...
        <Table>
          ...
          <TableBody>
            {availability.length === 0 ? (
              <TableRow><TableCell colSpan={4}>No availability posted yet.</TableCell></TableRow>
            ) : availability.map((a) => (
              <TableRow key={a.id}>
                ...
                <TableCell>
                  <Chip label={a.status} color={availabilityColor[a.status] || 'default'} size="small" />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
