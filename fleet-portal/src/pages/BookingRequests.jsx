...
import {
  Box, Typography, Paper, Grid, Button, Chip, CircularProgress, Alert,
} from '@mui/material';
...
export default function BookingRequests() {
  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight={700} gutterBottom>Booking Requests</Typography>

      {loading && <CircularProgress />}
      {error && <Alert severity="warning" sx={{ mb: 2 }}>{error}</Alert>}

      {!loading && bookings.length === 0 && !error && (
        <Alert severity="info">No pending booking requests right now.</Alert>
      )}
          ...
          <Grid item xs={12} md={6} key={b.id}>
            <Paper sx={{ p: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                ...
                <Chip label="Awaiting your decision" color="warning" size="small" />
              </Box>
